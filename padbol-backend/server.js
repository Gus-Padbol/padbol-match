...
/** `jugadores_perfil.deportes_preferidos` → lista canónica para contexto IA y tools. */
function chatIaNormalizeDeportesPreferidosDb(raw) {
  if (raw == null) return [];
  let arr = [];
  ...
  const seen = new Set();
  for (const x of arr) {
    const k = normalizeTorneoDeporteForDb(x);
    if (!TORNEO_DEPORTE_VALID.has(k) || seen.has(k)) continue;
    seen.add(k);
    out.push(k);
  }
  return out;
...
function chatIaAnthropicToolsDefinition() {
  return [
    ...
    },
    {
      ...
      description:
        'Lists clubs from an INNER JOIN sedes↔canchas: only rows where canchas.deporte matches the requested sport AND canchas.estado is activa (DB column). canchas_por_deporte is NOT used. If the user omits a sport but Context usuario_logueado.deportes_preferidos is set, use the first preferred sport. If sedes is empty, say honestly there are no clubs with that sport for that city/area (or nearby)—offer to search another city/region—and do NOT suggest clubs from Context sedes_resumen or listar_sedes unless they appear in this tool result. Optional ciudad/pais (ilike on sedes). Optional lat/lng for distance sort when no ciudad/pais.',
      input_schema: {
        type: 'object',
        properties: {
          deporte: {
            type: 'string',
            description:
              'Required unless Context usuario_logueado.deportes_preferidos supplies a default: padbol, padel, futbol, tenis, pickleball, squash, futbol_5, futbol_7',
          },
  ...
  try {
    ...
    if (toolName === 'buscar_sedes_por_deporte') {
      ...
      if (!depCanon) {
        return {
          ok: false,
          error: 'Parámetro deporte requerido (padbol, padel, futbol, tenis, pickleball, squash, futbol_5, futbol_7), o cargá deportes preferidos en tu perfil.',
        };
      }
...
async function buildChatIAContextPayload(supabaseClient, authUser, localeUiRaw) {
  ...
  if (authUser?.email) {
    ...
    const perfilQ = await supabaseClient
      .from('jugadores_perfil')
      .select(
        'nombre,nombre_saludo,email,ciudad,pais,whatsapp,sede_id,user_id,alias,apodo,apellido,deportes_preferidos'
      )
      .eq('email', em)
      .maybeSingle();
    ...
    usuarioBloque = {
      ...
      user_id: uid,
      perfil,
      deportes_preferidos: chatIaNormalizeDeportesPreferidosDb(perfil?.deportes_preferidos),
      reservas_ultimas_3,
...
function buildChatIaBootstrapPayload(ctx) {
  ...
  const deportes_preferidos = chatIaNormalizeDeportesPreferidosDb(
    u?.deportes_preferidos ?? u?.perfil?.deportes_preferidos
  );
  return {
    idioma: l,
...
STYLE:
- Default: maximum 3 short lines. No long paragraphs or bullet lists.

BREVITY WITH CHIPS OR FRANJAS (critical):
- If the app will show time-slot chips, sport-choice chips, or morning/afternoon/evening franja controls right after your message, you MUST write at MOST 2 lines of plain text BEFORE that UI—prefer exactly 1 line. No paragraphs. Do NOT repeat in prose what the chips already show (individual start times, long hour lists, "every 30 minutes from…", number of free courts per slot, etc.).
- After consultar_disponibilidad returns slots: your text is ONLY one line that identifies club + sport (if filtered) + day when helpful, ending with a colon or dash; then the user sees the chips. Nothing else—do NOT list hora_inicio values in the message.
- After requiere_elegir_deporte / sport chips: at most one short question line + the chips carry the options; do not paste the full slug list in a paragraph.
- Correct examples: "Mañana en La Meca — turnos disponibles:" / "Club Demo Multi-Sport tiene tenis mañana:" / "No hay disponibilidad para esa fecha."
- Wrong example: a long paragraph claiming "excelente disponibilidad" and enumerating half-hour slots from 8:00 to 21:30 and many courts—never do that; the UI is enough.

- Act without asking for confirmation when tools already returned data: give names, dates, and key facts concisely—for availability when the client shows slot or sport chips, follow BREVITY WITH CHIPS OR FRANJAS only (one line; never list individual hora_inicio or slug lists in prose).
...
- When the user asks for free slots, horarios, turnos or disponibilidad, you MUST call consultar_disponibilidad (after listar_sedes or buscar_sedes_por_deporte if needed) and answer ONLY with the tool result—except when disponibilidad_sede_implicita.fuente is ninguna and they have not specified a club yet: then ask which city/club first (no tool). Another exception: if the tool returns slots empty but requiere_elegir_deporte with deportes_disponibles, one short line to invite a choice (chips show sports; do not invent slots). Include deporte when the user names a sport OR when a preferred sport applies at that sede as in the Tools rule. For "mañana"/"tomorrow" pass that word as fecha or use fecha_mañana_art from the Context JSON. When slots are non-empty, your reply text is ONE line (club + sport if relevant + day) then the client shows chips—do NOT list times in the message (see BREVITY WITH CHIPS OR FRANJAS).
