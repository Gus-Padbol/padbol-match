...
const fillEnglishGaps = (resolved, generated, englishSource) => {
  const output = { ...resolved };
  Object.entries(generated || {}).forEach(([key, value]) => {
    if (!Object.hasOwn(output, key) || output[key] === englishSource[key]) output[key] = value;
  });
  return output;
};
...
const generatedQuality = readJson('src/i18n/generatedQualityOverrides.json');
...
const report = codes.map((code) => {
  ...
  const completed = {
    ...fillEnglishGaps(direct, flatten(generatedGaps[code] || {}), english),
    ...flatten(generatedQuality[code] || {}),
  };
  directByCode[code] = completed;
  const covered = englishKeys.filter((key) => Object.hasOwn(completed, key));
  return {
    code,
    covered: covered.length,
    total: englishKeys.length,
    missing: englishKeys.length - covered.length,
    identicalToEnglish: covered.filter((key) => completed[key] === english[key]).length,
  };
});
