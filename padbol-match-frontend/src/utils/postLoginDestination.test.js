...
describe('resolveRoleAwarePostLoginPath', () => {
  ...
  test('el primer ingreso luego del registro mínimo entra directamente al hub', async () => {
    await expect(
      resolveRoleAwarePostLoginPath('/', { access_token: 'registro-token' }, async () => ({ rol: 'jugador' })),
    ).resolves.toBe('/hub');
  });
