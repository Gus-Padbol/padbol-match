...
export default function AdminDashboard({
  ...
  const [pcSedeId, setPcSedeId] = useState(sedeIdKey || (sedeId != null ? String(sedeId) : ''));
  ...
  function resolvePcSedeId() {
    if (esAdminClub && sedeIdKey) return sedeIdKey;
    if (esAdminClub && sedeId != null && sedeId !== '') return String(sedeId);
    if (pcNeedsSelector) return pcSedeId;
    return sedeIdKey || (sedeId != null ? String(sedeId) : pcSedeId);
  }
  ...
  useEffect(() => {
    if (sedeIdKey && !pcSedeId) setPcSedeId(String(sedeIdKey));
    else if (sedeId != null && sedeId !== '' && !pcSedeId) setPcSedeId(String(sedeId));
  }, [sedeIdKey, sedeId, pcSedeId]);
  ...
  const patchSurgeActivoMaster = async (activo) => {
    setMiSedeForm((p) => ({ ...p, surge_activo: activo }));
    if (!sedeId || !session?.access_token) return;
    setSurgeActivoSaving(true);
    try {
      const res = await fetch(`${apiBaseUrl}/api/sedes/${sedeId}`, {
        method: 'PATCH',
        headers: {
        ...
        body: JSON.stringify({ surge_activo: activo }),
      });
      const data = await res.json().catch(() => ({}));
  ...
  const saveFranja = async () => {
    ...
    try {
      const row = {
        ...
        deporte: franjaDraft.deporte,
        dia_semana: franjaDraft.dia_semana === '' ? null : Number(franjaDraft.dia_semana),
        hora_inicio: franjaDraft.hora_inicio,
        hora_fin: franjaDraft.hora_fin,
        precio_60min: franjaDraft.precio_60min ? Number(franjaDraft.precio_60min.replace(/\./g, '')) : null,
        precio_90min: franjaDraft.precio_90min ? Number(franjaDraft.precio_90min.replace(/\./g, '')) : null,
        precio_120min: franjaDraft.precio_120min ? Number(franjaDraft.precio_120min.replace(/\./g, '')) : null,
        activo: true,
      };
  ...
  const deleteFranja = async (id) => {
    const { error } = await supabase.from('franjas_precio').update({ activo: false }).eq('id', id);
  ...
  return (
    ...
    >
        ...
        <div className="section">
          ...
          >
            ...
            <div style={{ display: 'grid', gap: '14px', marginBottom: '18px' }}>
              ...
              <label style={{ display: 'grid', gap: '6px' }}>
                <span style={{ fontWeight: 600, fontSize: '14px' }}>{t('admin.scoreboard.court', 'Cancha')}</span>
                <input
                  type="text"
                  value={sbCancha}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px', marginBottom: '18px' }}>
              <label style={{ display: 'grid', gap: '6px' }}>
                <span style={{ fontWeight: 600, fontSize: '14px' }}>{t('admin.scoreboard.teamA', 'Nombre Equipo A')}</span>
                <input
                  type="text"
                  value={sbEquipoA}
              ...
              <label style={{ display: 'grid', gap: '6px' }}>
                <span style={{ fontWeight: 600, fontSize: '14px' }}>{t('admin.scoreboard.teamB', 'Nombre Equipo B')}</span>
                <input
                  type="text"
                  value={sbEquipoB}
            ...
            <button
              ...
              style={{
                padding: '12px 24px',
                background: sbCreating ? 'var(--text-muted)' : 'var(--accent)',
                color: '#fff',
                border: 'none',
                borderRadius: '8px',
                fontWeight: 700,
          ...
          {sbCreated ? (
            ...
            >
              <h3 style={{ margin: '0 0 14px' }}>{t('admin.scoreboard.linksTitle', 'Links del partido')}</h3>
              {(() => {
                const origin = typeof window !== 'undefined' ? window.location.origin : '';
                const tvLink = `${origin}/display/${sbCreated.sede_id}/scoreboard/${sbCreated.id}`;
                const arbiterLink = `${origin}/admin/scoreboard/${sbCreated.id}`;
                return (
                  <>
                    <div style={{ marginBottom: '12px' }}>
                      <span style={{ fontSize: '13px', display: 'block', marginBottom: '6px', color: 'var(--text-muted)' }}>
                        {t('admin.scoreboard.tvLink', 'Pantalla TV')}
                      </span>
                      <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                        <a href={tvLink} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent)', wordBreak: 'break-all' }}>
                          {tvLink}
                        </a>
                        <button
                          type="button"
                          onClick={() => copiarLinkScoreboard(tvLink, 'tv')}
                          style={{ padding: '6px 12px', borderRadius: '6px', border: '1px solid var(--border)', cursor: 'pointer', fontSize: '12px', fontWeight: 600 }}
                        >
                          {sbCopied === 'tv' ? t('admin.scoreboard.copied', '✓ Copiado') : t('admin.scoreboard.copy', 'Copiar')}
                        </button>
                      </div>
                    </div>
                    <div>
                      <span style={{ fontSize: '13px', display: 'block', marginBottom: '6px', color: 'var(--text-muted)' }}>
                        {t('admin.scoreboard.arbiterLink', 'Panel del árbitro')}
                      </span>
                      <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                        <a href={arbiterLink} style={{ color: 'var(--accent)', wordBreak: 'break-all' }}>
                          {arbiterLink}
                        </a>
                        <button
                          type="button"
                          onClick={() => copiarLinkScoreboard(arbiterLink, 'arbiter')}
                          style={{ padding: '6px 12px', borderRadius: '6px', border: '1px solid var(--border)', cursor: 'pointer', fontSize: '12px', fontWeight: 600 }}
                        >
                          {sbCopied === 'arbiter' ? t('admin.scoreboard.copied', '✓ Copiado') : t('admin.scoreboard.copy', 'Copiar')}
                        </button>
                      </div>
                    </div>
                  </>
      ...
      {activeTab === 'padcoins' && puedeVerPadCoins ? (() => {
        ...
        const puedeEditarParticipacion = isSuperAdmin || esAdminClub;
            ...
            <label style={{
              ...
              <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                <span style={{ fontWeight: 600, fontSize: '14px', color: 'var(--text-primary)' }}>
                  {esAdminClub
                    ? t('admin.padcoins.sedeParticipationClubToggle', 'Activar Beneficios Padbol en mi sede')
                    : t('admin.padcoins.sedeParticipationSuperToggle', 'Participación en Beneficios Padbol')}
                </span>
          ...
          <div className="section">
            ...
            <div style={{
              ...
              {isSuperAdmin && pcSedesParticipacionList.length > 0 ? (
                <div style={{ marginBottom: '20px', maxWidth: '720px' }}>
                  <div style={{
                    ...
                    <input
                      type="search"
                      value={pcSedeParticipacionBusqueda}
                      onChange={(e) => setPcSedeParticipacionBusqueda(e.target.value)}
                      placeholder={t('admin.padcoins.sedeParticipationSearch', 'Buscar por nombre o sede_id...')}
                      style={{ ...pcInp, flex: '1 1 220px', maxWidth: '320px' }}
                    />
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                      {[
                        { id: 'todas', label: t('admin.padcoins.sedeParticipationFilterAll', 'Todas') },
                        { id: 'activas', label: t('admin.padcoins.sedeParticipationFilterActive', 'Activas') },
                        { id: 'inactivas', label: t('admin.padcoins.sedeParticipationFilterInactive', 'Inactivas') },
                      ].map(({ id, label }) => {
                        const active = pcSedeParticipacionFiltro === id;
                        return (
                          <button
                            key={id}
                            type="button"
                            onClick={() => setPcSedeParticipacionFiltro(id)}
                            style={{
                              padding: '7px 12px',
                              borderRadius: '999px',
                  ...
                  <div style={{ overflowX: 'auto', maxHeight: '280px', overflowY: 'auto', border: '1px solid var(--border)', borderRadius: '10px' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                      <thead>
                        <tr style={{ borderBottom: '2px solid var(--border)', textAlign: 'left', position: 'sticky', top: 0, background: 'var(--bg-card)' }}>
                      ...
                      <tbody>
                        {pcSedesParticipacionFiltradas.length === 0 ? (
                          <tr>
                            <td colSpan={3} style={{ padding: '14px 10px', color: 'var(--text-muted)' }}>
                              {t('admin.padcoins.sedeParticipationNoResults', 'No hay sedes que coincidan con la búsqueda o el filtro.')}
                            </td>
                          </tr>
                          ...
                          return (
                            ...
                            >
                              ...
                              <td style={{ padding: '10px', color: 'var(--text-muted)', fontFamily: 'monospace', fontSize: '12px' }}>
                                {sid || '—'}
                              </td>
              ...
              {effectivePcSedeId && !pcSedeParticipacionLoading && !pcSedeParticipacionError ? (
                <div style={{
                  ...
                  {puedeEditarParticipacion ? renderPcParticipacionForm() : (
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px', marginBottom: '20px' }}>
              ...
              {effectivePcSedeId && !premioFormMode && puedeCrearPremio ? (
                ...
                >
                  + {t('admin.padcoins.newBenefit', 'Nuevo beneficio')}
                </button>
              ) : null}
            </div>
            ...
            {premioFormMode && effectivePcSedeId ? (
              ...
              >
                ...
                <div style={{ display: 'grid', gap: '14px' }}>
                  <label style={{ display: 'grid', gap: '6px' }}>
                    <span style={{ fontWeight: 600, fontSize: '14px' }}>{t('admin.padcoins.name', 'Nombre')} *</span>
                    <input
                      type="text"
                  ...
                  <label style={{ display: 'grid', gap: '6px' }}>
                    <span style={{ fontWeight: 600, fontSize: '14px' }}>{t('admin.padcoins.descriptionField', 'Descripción')}</span>
                    <textarea
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '14px' }}>
                    <label style={{ display: 'grid', gap: '6px' }}>
                      <span style={{ fontWeight: 600, fontSize: '14px' }}>{t('admin.padcoins.cost', 'Costo (PadCoins)')} *</span>
                      <input
                        type="number"
                        min="1"
                        step="1"
                    ...
                    <label style={{ display: 'grid', gap: '6px' }}>
                      <span style={{ fontWeight: 600, fontSize: '14px' }}>{t('admin.padcoins.stockTotal', 'Stock total')}</span>
                      <input
                        type="number"
                        min="0"
                        step="1"
                    ...
                    <label style={{ display: 'grid', gap: '6px' }}>
                      <span style={{ fontWeight: 600, fontSize: '14px' }}>{t('admin.padcoins.stockAvailable', 'Stock disponible')}</span>
                      <input
                        type="number"
                        min="0"
                        step="1"
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '14px' }}>
                    <label style={{ display: 'grid', gap: '6px' }}>
                      <span style={{ fontWeight: 600, fontSize: '14px' }}>{t('admin.padcoins.startDate', 'Fecha inicio')}</span>
                      <input
                        type="date"
                        value={premioForm.fecha_inicio}
                        onChange={(e) => setPremioForm((p) => ({ ...p, fecha_inicio: e.target.value }))}
                        style={pcInp}
                      />
                    </label>
                    <label style={{ display: 'grid', gap: '6px' }}>
                      <span style={{ fontWeight: 600, fontSize: '14px' }}>{t('admin.padcoins.endDate', 'Fecha fin')}</span>
                      <input
                        type="date"
                        value={premioForm.fecha_fin}
                        onChange={(e) => setPremioForm((p) => ({ ...p, fecha_fin: e.target.value }))}
                        style={pcInp}
                      />
                    </label>
                  </div>
                  <label style={{ display: 'grid', gap: '6px' }}>
                    <span style={{ fontWeight: 600, fontSize: '14px' }}>{t('admin.padcoins.conditions', 'Condiciones')}</span>
                    <textarea
                      value={premioForm.condiciones}
                      onChange={(e) => setPremioForm((p) => ({ ...p, condiciones: e.target.value }))}
                      rows={2}
                      placeholder={t('admin.padcoins.conditionsPlaceholder', 'Ej: Válido de lunes a viernes')}
                      style={{ ...pcInp, resize: 'vertical' }}
                    />
                  </label>
                ...
                <div style={{ display: 'flex', gap: '10px', marginTop: '18px', flexWrap: 'wrap' }}>
                  <button
                    ...
                    style={{
                      padding: '10px 20px',
                      background: premioSaving ? '#94a3b8' : 'var(--accent)',
                      color: '#fff',
                      border: 'none',
                      borderRadius: '8px',
                      fontWeight: 700,
                  ...
                  >
                    <div style={{ flex: 1, minWidth: '220px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap', marginBottom: '6px' }}>
                        <strong style={{ fontSize: '16px', color: 'var(--text-primary)' }}>{premio.nombre}</strong>
                        <span style={{
                          background: premio.activo !== false ? '#dcfce7' : 'var(--bg-page)',
                          color: premio.activo !== false ? '#166534' : 'var(--text-muted)',
                          borderRadius: '12px',
                          padding: '2px 10px',
                          fontSize: '11px',
                          fontWeight: 700,
                          ...
                          {premio.activo !== false
                            ? t('admin.padcoins.statusActive', 'Activo')
                            : t('admin.padcoins.statusInactive', 'Inactivo')}
                        </span>
                      </div>
                      {premio.descripcion ? (
                        <p style={{ margin: '0 0 8px', color: 'var(--text-muted)', fontSize: '13px' }}>{premio.descripcion}</p>
                      ) : null}
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', fontSize: '13px', color: 'var(--text-secondary)' }}>
                        <span><strong>{t('admin.padcoins.cost', 'Costo')}:</strong> {premio.costo_padcoins} PadCoins</span>
                        {premio.stock_total != null ? (
                          <span><strong>{t('admin.padcoins.stockTotal', 'Stock total')}:</strong> {premio.stock_total}</span>
                        ) : null}
                        {premio.stock_disponible != null ? (
                          <span><strong>{t('admin.padcoins.stockAvailable', 'Disponible')}:</strong> {premio.stock_disponible}</span>
                        ) : null}
                        {premio.fecha_inicio ? (
                          <span><strong>{t('admin.padcoins.startDate', 'Desde')}:</strong> {String(premio.fecha_inicio).slice(0, 10)}</span>
                        ) : null}
                        {premio.fecha_fin ? (
                          <span><strong>{t('admin.padcoins.endDate', 'Hasta')}:</strong> {String(premio.fecha_fin).slice(0, 10)}</span>
                        ) : null}
                      </div>
                      {premio.condiciones ? (
                        <p style={{ margin: '8px 0 0', fontSize: '12px', color: 'var(--text-muted)' }}>
                          <strong>{t('admin.padcoins.conditions', 'Condiciones')}:</strong> {premio.condiciones}
                        </p>
                      ) : null}
                    </div>
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                      ...
                      >
                        {t('admin.padcoins.deactivate', 'Desactivar')}
                      </button>
                    </div>
                  </div>
                ))}
