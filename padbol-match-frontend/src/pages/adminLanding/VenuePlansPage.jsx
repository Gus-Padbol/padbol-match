...
function formatCommission(value, locale = 'en-US', notDefined = 'To be defined') {
  if (value == null || value === '') return notDefined;
  const number = Number(value);
    ...
    <article className={`venue-plans__card${plan.featured ? ' venue-plans__card--featured' : ''}`}>
      ...
      <div className="venue-plans__price-block">
        <p className="venue-plans__price">
          {plan.contactOnly ? plan.ctaLabel : (
            <><span>{prefix}</span>{monthly}<small>{copy.perMonth}</small></>
          )}
        </p>
        {!plan.contactOnly && annual != null && Number(plan.annualAmount) !== Number(plan.monthlyAmount) ? (
      ...
      <div className="venue-plans__commission">
        ...
        <strong>{plan.contactOnly ? copy.notDefined : `${commissionPrefix}${formatCommission(plan.commissionPercent, locale, copy.notDefined)}`}</strong>
        {!plan.contactOnly ? <small>{copy.commissionNote}</small> : null}
      </div>
...
export default function VenuePlansPage({ catalogOverride = null }) {
  ...
  const padbolOwnerTitleMatch = String(copy.padbolOwnerTitle || '').match(/^(.*?)(Padbol Courts\s*\?)$/i);
        ...
        <section className="venue-plans__padbol-owner" aria-labelledby="padbol-owner-title">
          <div className="public-site__shell venue-plans__padbol-owner-grid">
            <div>
              ...
              <h2 id="padbol-owner-title">
                {padbolOwnerTitleMatch ? (
                  <>{padbolOwnerTitleMatch[1]}<span className="venue-plans__padbol-courts">{padbolOwnerTitleMatch[2]}</span></>
                ) : copy.padbolOwnerTitle}
              </h2>
