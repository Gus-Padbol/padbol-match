...
test.each(PADBOL_LANGUAGE_CODES)('%s shows no Padbol Match commission to the player', (code) => {
  const t = i18n.getFixedT(code);
  ['cargoServicio', 'serviceFee', 'playerCommissionZero', 'noServiceFeeVenue'].forEach((key) => {
    expect(t(`reservas.${key}`)).toMatch(/(?:0|۰)\s*[%٪]/);
    expect(t(`reservas.${key}`)).not.toMatch(/(?:3|۳)\s*[%٪]/);
  });
});
